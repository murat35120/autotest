...
function *gen(data, writer, comment){ //команда с переходом по времени выводом результата в окно
	...
	let timer=setTimeout(()=>{control_stand[writer]( txt, comment); this.next();}, twa); //this.next - метод объекта аа
	let answer = yield; //получаем ответ от устройства
	while(true) {
		txt = control_stand.buff_sum([txt, answer]);
		if(txt.length>=17){
			clearTimeout(timer);
			let timer1=setTimeout(()=>{control_stand[writer]( txt, comment); this.next();}, ppo);//отправляем новый запрос через ppo = паузу после получения отвера
		}else{		
			if(txt.length==13){
				if(txt[1]==1){
					clearTimeout(timer);
					let timer1=setTimeout(()=>{control_stand[writer]( txt, comment); this.next();}, ppo);
				}
			}
			if(txt.length==8){
				if(txt[1]==6||txt[3]==16){
...
function *scan(data, writer, comment){ //команда с переходом по команде вложенной функции
	...
	let timer=setTimeout(()=>{this.next();}, twa);
	let answer = yield; //получаем ответ от устройства
	while(true) {
...
function *show_cnt(data, writer, comment){ //команда с переходом по команде вложенной функции
	maker.writeObject(controllers, blocks.id.devicesControllers);
	let timer=setTimeout(()=>{this.next(); }, twa);
	yield;
}
function *log_writer(data, writer, comment){ //весь ответ в консоль
	let txt=new Uint8Array(0);
	this.writer.write(data); //отправляем подготовленное сообщение
	let timer=setTimeout(()=>{this.next();}, twa);
	let answer = yield; //получаем ответ от устройства
...
let control_stand={
	...
	converter_info(){
		let data = control_stand.buff_sum(new Uint8Array([0x69, 0x0D])); //i + enter = информация о конвертере
		//aa.add(log_writer, data, "finder1", "info1",""); //func,  data, writer, comment, callback) {//функция добавления в очередь
		return data;
	},
	read(addess){
		if(typeof addess=="object"){addess=abonent.address;}
	...
	recovery(){ //подключение (выбор) ком порта (канал ввода вывода)
		let filters = [
		...
		(async () => { //ввод и вывод данных порта
			...
			let port = await navigator.serial.requestPort(); //выбираем порт
			await port.open(settings); //настройки
			//for(let i in links.click){  //-  это вероятно понадобится
	...
	finder(data, comment){ //если ответ есть, добавляем адрес в список
		if(data[1]){
			if(!(data[0] in controllers)){
				...
				control_stand.get_info(data[0]); //получаем информацию о новом контроллере
			} else{
				if(data[1]==1){//получаем информацию о новом контроллере
					...
					if(data[0]<=1){
						control_stand.new_addess(data[0], max_addess()); //(old_adr, new_ard)
					}
					//control.controllers_button(data[0]);
				}
			}
			//pole.innerText=data.subarray(0,11);//data;
			//console.log(data.subarray(0,11));
		}
	},
