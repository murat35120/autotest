...
let maker={
	...
	clickItem: new OneType(['dataset_click', 'text', 'dataset_item'],{className:'buttons'}, blocks ),
	...
	selectBlocks: new OneType(['dataset_read', 'list', 'className'],{tag:'select'}, blocks ),	
	...
	writeList(list, parentBlk, feld_id){//ожидается [{},{},{}]
	    let i=0;
		for(i=0; i<list.length; i++){
			let k=maker.ligth.make('', parentBlk);
			for(let j in list[i]){
				maker.ligth.make(list[i][j], k);
    			if(j==feld_id){
