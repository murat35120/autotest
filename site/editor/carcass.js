...
let carcass_functions={
    ...
    allInput(parent){//на выходе {}
		let obj={};
        let arr=parent.querySelectorAll('input[data-read]');
        for (let i=0; i<arr.length; i++){
			let name =arr[i].dataset.read;
	...
	recovery(link){
		...
		(async () => { //ввод и вывод данных порта
			...
			carcass_functions.portWriter("i","parse_i");
			abonent.adapter={};
	...
	portWriter(data, func){
	    let ss=data+"\r";
	    let encoded = new TextEncoder("windows-1251").encode(ss);
	    carcass_functions.aa.writer.write(encoded);
	    carcass_functions.func=func;
	},
	writeInfo(data){
		if(carcass_functions[carcass_functions.func]){
			carcass_functions[carcass_functions.func](data);
		}else{
			console.log('data');
			console.log(data);
			console.log('------');
	...
	parse_i(data){
		clearTimeout(carcass_functions.tmt);
		carcass_functions.txt=carcass_functions.txt+data;
		carcass_functions.tmt=setTimeout(carcass_functions.parse_i_1, 100);
	},
	parse_i_1(){
		let data=carcass_functions.txt;
		console.log(data);
		carcass_functions.txt='';
		let arr0=data.split('USB');
		if(arr0[1]){
			let arr=arr0[1].split('S/N:');
			abonent.adapter.name=arr[0];
			blocks.read.devicesadaptermodel.innerText=abonent.adapter.name;
			if(arr[1]){
				let arr1=arr[1].split('[');
				abonent.adapter.number=arr1[0];
				blocks.read.devicesadapternumber.innerText=abonent.adapter.number;
				if(arr1[1]){
					let arr3=arr1[1].split(']');
					abonent.adapter.type=arr3[0];
					if(arr3[1]){
						let arr4=arr3[1].split('Software version:');
						if(arr4[1]){
							let arr5=arr4[1].split(',');
							abonent.adapter.version=arr5[0];
							blocks.read.devicesadapterversion.innerText=abonent.adapter.version;
						}
					}
				}
			}
...
let maker={
	...
	name_idText(feld, parent, set){
			maker.addBlocks.prepare([[maker.name_id_text.prepare(feld), set]], parent);
		},
	...
	writeList(list, parentBlk, feld_id, feld_click, names){//ожидается [{},{},{}] names=[]
		parentBlk.innerHTML='';
		let i=0;
		if(list){
	...
	writeObject(obj, parentBlk, feld_id, feld_click){//ожидается {{},{},{}}
		parentBlk.innerHTML='';
		for(let i in obj){
			let k=maker.ligth.make('', parentBlk, i, 'directionColumn');
			for(let j in obj[i]){
				blk=maker.name_idText([j,['',obj[i][j]]], k);
			}
		}
	},
